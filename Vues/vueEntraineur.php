...
<?php
    ...
    echo "<table class=etat>";
    echo "<tr><th>Categorie</th><th>Competition</th><th>Equipe</th><th>Equipe adverse</th><th>Date</th><th>Heure</th><th>Terrain</th><th>Site</th><th>Convocation</th></tr>";
        foreach($tabRencontre as $a){
            echo "<tr id=\"$a[id]\"><td>SENIORS</td><td>$a[competition]</td><td>$a[equipe]</td><td>$a[equipe_adverse]</td><td>$a[date]</td><td>$a[heure]</td><td>$a[terrain]</td><td>$a[site]</td><td><input type=\"button\" value=\"ajouter\" class=\"ajout\"</td></tr>";
        }
    echo "</table>";

    echo "<h1 id=\"conv\">CONVOCATION</h1>";
    echo "<table class=etat>";
    echo "<tr><th>Publier</th><th>Date</th><th>Competition</th><th>Equipe Adv</th><th>Site</th><th>terrain</th><th>Heure</th><th>Rdv</th><th>Equipe</th><th>Selection</th><th>Joueur1</th><th>Joueur2</th><th>Joueur3</th><th>Joueur4</th><th>Joueur5</th><th>Joueur6</th><th>Joueur7</th><th>Joueur8</th><th>Joueur9</th><th>Joueur10</th><th>Joueur11</th><th>Joueur12</th><th>Joueur13</th><th>Joueur14</th></tr>";
        foreach($tabconvoc as $a){
            $tabjoueurs = joueursDisponiblesdate($a['date']);
            echo "<tr><td>
            <form action=\"CEntraineur.php\" method=\"POST\" onsubmit=\"return confirm('Etes vous sûr de vouloir publier cette convocation ?');\">
                <input type=\"hidden\" value=\"$a[id]\" name=\"id\">
                <input type=\"submit\" value=\"Publier\" name=\"pub\">
            </form> 
            </td><td>$a[date]</td><td>$a[competition]</td><td>$a[equipe_adverse]</td><td>$a[site]</td><td>$a[terrain]</td><td>$a[heure]</td><td>$a[rdv]</td><td>$a[equipe]</td><td>
            <form action=\"CEntraineur.php\" method=\"POST\" onsubmit=\"return confirm('Attention vous ne pourrez plus changer ces informations etes vous sur de vouloir entrer ces joueurs, si vous voulez pouvoir reconvoquer ces joueurs vous devrez les rendre disponibles dans la table des absences ( voir plus haut )');\">
                <fieldset>
                    <legend>Selectionnez vos joueurs</legend>
                    <input type=\"hidden\" name=\"date\" value=\"$a[date]\">
                    <input type=\"hidden\" name=\"id\" value=\"$a[id]\">
                    <select name=\"ary[]\" class=\"selection\" multiple>$tabjoueurs</select>
                    <input type=\"textarea\" name=\"texta\" placeholder=\"Precisions sur le rdv\">
                    <input name=\"ajoutJoueur\" type=\"submit\" value=\"ENVOYER\" class=\"modRenc\">
                </fieldset>
            </form>
        </td><td>$a[joueur1]</td><td>$a[joueur2]</td><td>$a[joueur3]</td><td>$a[joueur4]</td><td>$a[joueur5]</td><td>$a[joueur6]</td><td>$a[joueur7]</td><td>$a[joueur8]</td><td>$a[joueur9]</td><td>$a[joueur10]</td><td>$a[joueur11]</td><td>$a[joueur12]</td><td>$a[joueur13]</td><td>$a[joueur14]</td></tr>";
        }
    echo "</table>";
